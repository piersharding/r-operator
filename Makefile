...
TAG ?= latest
REPLICAS ?= 2
INGRESS_HOST ?= testapp.rapp.local

.PHONY: k8s show lint deploy delete logs describe namespace test clean metalogs rgdal localip help
.DEFAULT_GOAL := help
...
rgdal:
	docker build \
	  -t rgdal:latest -f rgdal/Dockerfile.rgdal ./rgdal
	docker tag rgdal:latest piersharding/rgdal:latest
	docker push piersharding/rgdal:latest
...
logs: ## operator logs
	kubectl get pods -l \
	app.kubernetes.io/instance=$(HELM_RELEASE),app.kubernetes.io/name=r-controller-r-operator \
